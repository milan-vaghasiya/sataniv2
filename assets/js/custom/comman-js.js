...
$(document).ready(function(){
    ...
    $(document).on("change","#terms_title",function(){
    	var term_id = $(this).val();
    	if(term_id){			
            $.ajax({
    			url: base_url + 'terms/getTermsData',
    			data:{id:term_id},
    			type: "POST",
    			dataType:"json",
            }).done(function(data){
    			tinyMCE.get('conditions').setContent(data.conditions);
    		});                              
    	}
    });
...
function customStore(postData){
	...
	$.ajax({
		...
		if(resFunctionName != ""){
			if(formClose){ 
				initSelect2(); $('#'+formId)[0].reset(); closeModal(formId);
				Swal.fire({ icon: 'success', title: data.message}); 
			}
			...
			if(data.status==1){
				initTable(); initSelect2(); $('#'+formId)[0].reset(); closeModal(formId);
				Swal.fire({ icon: 'success', title: data.message});
...
function printTag(data){
	...
	$.ajax({
		...
		if(response.status==0){
		    ...
		    printContents = response.printHtml;console.log(printContents);
