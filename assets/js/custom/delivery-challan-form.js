...
function createInvoice(){
	...
	$(".orderItem:checked").map(function() {
		...
		row.price = (parseFloat(row.price) > 0)?row.price:row.price;
		row.stock_eff = 1;
...
function AddRow(data) {
	...
	var natureProcessInput = $("<input/>", { type: "hidden", name: "itemData["+itemCount+"][nature_of_process]", value: data.nature_of_process });
	var packingTypeInput = $("<input/>", { type: "hidden", name: "itemData["+itemCount+"][packing_type]", value: data.packing_type });
    ...
    cell.append(natureProcessInput);
...
function Edit(data, button) {
	...
	setTimeout(function(){
		if(data.stock_eff == 1){
			...
			console.log(data.batch_detail);
			var qty_readonly = '';
			var batchDetail = {'postData':{'item_id': data.item_id, 'id' : data.id, 'batchDetail': data.batch_detail,'qty_readonly':qty_readonly},'table_id':"batchDetail",'tbody_id':'batchTrans','tfoot_id':'','fnget':'getBatchWiseItemStock'};
...
function resItemDetail(response = ""){
    if(response != ""){
		...
		var batchDetail = {'postData':{'item_id': itemDetail.id, 'id' : ''},'table_id':"batchDetail",'tbody_id':'batchTrans','tfoot_id':'','fnget':'getBatchWiseItemStock'};
		getTransHtml(batchDetail);
    }else{
