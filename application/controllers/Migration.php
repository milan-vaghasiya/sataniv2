...
class Migration extends MY_Controller{
	...
	public function addColumnInTable(){
        $result = $this->db->query("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'nativebi_sataniv2' AND TABLE_NAME NOT IN ( SELECT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME = 'cm_id' AND TABLE_SCHEMA = 'nativebi_sataniv2' )")->result();
    ...
    public function migrateSrStock(){
        try{
            $this->db->trans_begin();

            $this->db->reset_query();
            $this->db->select("entry_type,ref_date,ref_no,main_ref_id,child_ref_id,stock_transaction.location_id,stock_transaction.batch_no,heat_no,stock_transaction.party_id,item_id,p_or_m,SUM(stock_transaction.qty) as stock_qty,stock_transaction.size,stock_transaction.price,stock_transaction.created_by,stock_transaction.created_at,stock_transaction.cm_id,item_master.item_type");
            $this->db->join('item_master','item_master.id=stock_transaction.item_id');
            $this->db->where('entry_type',9);
            $this->db->where('stock_transaction.qty',1);
            $this->db->where('item_master.item_type',2);
            $this->db->where('stock_transaction.is_delete',0);
            $this->db->group_by('item_id,batch_no,location_id,child_ref_id');
            ...
            foreach($result as $row):
                ...
                $this->db->where('child_ref_id',$row->child_ref_id);
                $this->db->where('ref_no',$row->ref_no);
                $this->db->where('ref_date',$row->ref_date);
                //$this->db->update('stock_transaction',['is_delete'=>9]);
                ...
                $postData = [
                    ...
                    'ref_no' => $row->ref_no,
                    'main_ref_id' => $row->main_ref_id,
                    'child_ref_id' => $row->child_ref_id,
                    ...
                    'cm_id' => $row->cm_id,
                    'remark' => 'SR WISE STOCK MIGRATE (JP@12112024)'
                ];
                
                $this->db->reset_query();
                //$this->db->insert('stock_transaction',$postData);
            ...
            if($this->db->trans_status() !== FALSE):
                //$this->db->trans_commit();
                echo $i." Stock Migration Success.";
            endif;
        }catch(\Throwable $e){
            $this->db->trans_rollback();
            echo $e->getMessage();exit;
    ...
    public function migrateEmployeeMaster(){
        try{
            $this->db->trans_begin();

            $this->db->reset_query();
            $this->db->select("id,emp_code");
            // $this->db->where('is_delete',1);
            $result = $this->db->get('employee_master')->result();
            $i =0;$j=0;
            foreach($result as $row):
                $this->db->reset_query();
                $this->db->select("employee_master_old.*");
                $this->db->where('emp_code',$row->emp_code);
                $oldEmpData = $this->db->get('employee_master_old')->row();

                
                if(!empty($oldEmpData->id)){
                    $this->db->reset_query();
                    $this->db->where('id',$row->id);
                    $this->db->update('employee_master',['old_id' =>  $oldEmpData->id ]);

                    $this->db->reset_query();
                    $this->db->where('id',$oldEmpData->id);
                    $this->db->update('employee_master_old',['new_id' =>  $row->id ]);
                    $i++;
                }
                $j++;
            endforeach;
            ...
            if($this->db->trans_status() !== FALSE):
                // $this->db->trans_commit();
                echo "Employee Migrated From ".$j." To ".$i;
            endif;
        }catch(\Throwable $e){
            $this->db->trans_rollback();
            echo $e->getMessage();exit;
    ...
    public function migrateNewEmployeeFromV2(){
        try{
            $this->db->trans_begin();
            ...
            $result = $this->db->get('employee_master_old')->result();
            $i =0;$j=0;
            foreach($result as $row):
                $this->db->reset_query();
                $oldId = $row->id;
                unset($row->id,$row->new_id,$row->salary_duration,$row->salary_code,$row->cl_charge,$row->cd_charge,$row->traveling_charge,$row->lq_status,$row->party_id);
                $insertEmp = (array)$row;
                $insertEmp['old_id'] =$oldId;
                $this->db->reset_query();
                $this->db->insert('employee_master',$insertEmp);

                $this->db->reset_query();
                $this->db->where('id',$oldId);
                $this->db->update('employee_master_old',['new_id' =>  $this->db->insert_id() ]);
                $j++;
            endforeach;

            if($this->db->trans_status() !== FALSE):
                $this->db->trans_commit();
                echo "Employee Migrated  ".$j;
            endif;
        }catch(\Throwable $e){
            $this->db->trans_rollback();
            echo $e->getMessage();exit;
    ...
    /* ----------------------------------------------------------- */
    // Created by migrate krva mate new field add krvu
    public function addNewEmpColumnInTable(){

        $result = $this->db->query("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'sataniv2' AND TABLE_NAME NOT IN ( SELECT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME = 'new_emp_id' AND TABLE_SCHEMA = 'sataniv2' )")->result();

        foreach($result as $row):
            //New new_emp_id For created By
            $this->db->query("ALTER TABLE ".$row->TABLE_NAME." ADD `new_emp_id` INT NOT NULL DEFAULT '0' AFTER `created_by`;");
        endforeach;

        echo "success";exit;
    }

    /** New created by ma value add krvi */
    public function migrateNewCreatedByAllTbl() {
        try{
            $this->db->trans_begin();
            ...
            foreach ($tables as $table) {
                // Skip
                if(in_array($table,['employee_master_old','employee_master_old_2','department_master','employee_master','emp_designation','shift_master','leave_master',' leave_type','leave_master','facility_master','holidays','attendance_policy','alog_summary','shift_master','attendance_log','emp_shiftlog','salary_structure','device_master','emp_docs','emp_nomination_detail','emp_facility','emp_education_detail','emp_docs_detail','canteen_appointment','canteen_trans'])){ continue; }

                $fields = $this->db->list_fields($table);
                if (in_array('created_by', $fields) && in_array('new_emp_id', $fields)) {
                    $sql = "
                        UPDATE `$table` t
                        JOIN employee_master_old_2 ON t.created_by = employee_master_old_2.v2_emp_id
                        SET t.new_emp_id = employee_master_old_2.id
                    ";
                    $this->db->query($sql);
                    ...
                    echo "Skipped table (missing columns): $table<br>";
                }
            }
    ...
    /** New value created by ma update kri column delete kri */
    public function migrateCreatedByAllTbl() {
        try{
            $this->db->trans_begin();
            ...
            foreach ($tables as $table) {
                ...
                if (in_array('created_by', $fields) && in_array('new_emp_id', $fields)) {
                    $sql = "
                        ...
                        SET t.created_by = t.new_emp_id
                    ";
                    $this->db->query($sql);

                    echo "Updated table: $table<br>";
                     $this->db->reset_query();
                     $this->db->query("ALTER TABLE `$table` DROP COLUMN new_emp_id;");
                    $i++;
                } else {
                    echo "Skipped table (missing columns): $table<br>";
                }
    ...
    /** New field operator_id & emp_id & updated_by add kri */
    //Operator_id,emp_id,updated_by
    public function addOtherField() {
        try{
            ...
            foreach ($tables as $table) {
                // Skip
                if(in_array($table,['employee_master_old','employee_master_old_2','department_master','employee_master','emp_designation','shift_master','leave_master',' leave_type','leave_master','facility_master','holidays','attendance_policy','alog_summary','shift_master','attendance_log','emp_shiftlog','salary_structure','device_master','emp_docs','emp_nomination_detail','emp_facility','emp_education_detail','emp_docs_detail','canteen_appointment','canteen_trans','leave_master_old'])){ continue; }

                $fields = $this->db->list_fields($table);
                if (in_array('operator_id', $fields) && !in_array('new_operator_id', $fields)) {
                    $this->db->reset_query();
                    $sql = "ALTER TABLE `$table` ADD `new_operator_id` INT NOT NULL DEFAULT '0' AFTER `operator_id`";
                    $this->db->query($sql);

                    echo "Operator Updated table: $table<br>";
                    $i++;
                } 

                if (in_array('emp_id', $fields) && !in_array('new_employee_id', $fields)) {
                
                    $this->db->reset_query();
                    $sql = "ALTER TABLE `$table` ADD `new_employee_id` INT NOT NULL DEFAULT '0' AFTER `emp_id`";
                    $this->db->query($sql);

                    echo "EMP Id Updated table: $table<br>";
                    $i++;
                } 

                if (in_array('updated_by', $fields) && !in_array('new_updated_by', $fields)) {
                
                    $this->db->reset_query();
                    $sql = "ALTER TABLE `$table` ADD `new_updated_by` INT NOT NULL DEFAULT '0' AFTER `updated_by`";
                    $this->db->query($sql);
    ...
    /** New fields  ma value add krvi */
    public function UpdateOtherFieldAllTbl() {
        try{
            $this->db->trans_begin();
            ...
            foreach ($tables as $table) {
                ...
                $this->db->reset_query();
                    $sql = "
                        UPDATE `$table` t
                        JOIN employee_master_old_2 ON t.operator_id = employee_master_old_2.v2_emp_id
                        SET t.new_operator_id = employee_master_old_2.id
                    ";
                    $this->db->query($sql);
                ...
                if (in_array('emp_id', $fields) && in_array('new_employee_id', $fields)) {
                    $this->db->reset_query();
                    $sql = "UPDATE `$table` t
                        JOIN employee_master_old_2 ON t.emp_id = employee_master_old_2.v2_emp_id
                        SET t.new_employee_id = employee_master_old_2.id";
                    $this->db->query($sql);
                ...
                if (in_array('updated_by', $fields) && in_array('new_updated_by', $fields)) {
                    $this->db->reset_query();
                    $sql = "UPDATE `$table` t
                        JOIN employee_master_old_2 ON t.updated_by = employee_master_old_2.v2_emp_id
                        SET t.new_updated_by = employee_master_old_2.id";
                    $this->db->query($sql);

                    echo "UPDATED BY Updated table: $table<br>";
                    $i++;
                } 
            }
    ...
    /**Original Field ma value updated krvi */
    public function migrateOtherFieldData() {
        try{
            $this->db->trans_begin();
            ...
            foreach ($tables as $table) {
                ...
                if (in_array('operator_id', $fields) && in_array('new_operator_id', $fields)) {
                    $sql = "
                        UPDATE `$table` t
                        SET t.operator_id = t.new_operator_id
                    ";
                    $this->db->query($sql);

                    echo "OPERATOR Updated table: $table<br>";
                     $this->db->reset_query();
                     $this->db->query("ALTER TABLE `$table` DROP COLUMN new_operator_id;");
                    $i++;
                } 
                ...
                if (in_array('emp_id', $fields) && in_array('new_employee_id', $fields)) {
                    $sql = "
                        UPDATE `$table` t
                        SET t.emp_id = t.new_employee_id
                    ";
                    $this->db->query($sql);

                    echo "EMP ID Updated table: $table<br>";
                     $this->db->reset_query();
                     $this->db->query("ALTER TABLE `$table` DROP COLUMN new_employee_id;");
                    $i++;
                } 
                ...
                if (in_array('updated_by', $fields) && in_array('new_updated_by', $fields)) {
                    $sql = "
                        UPDATE `$table` t
                        SET t.updated_by = t.new_updated_by
                    ";
                    $this->db->query($sql);

                    echo "UPDATED BY Updated table: $table<br>";
                     $this->db->reset_query();
                     $this->db->query("ALTER TABLE `$table` DROP COLUMN new_updated_by;");
                    $i++;
                } 
            }
    ...
    public function migrateAdminDataData() {
        try{
            $this->db->trans_begin();
            ...
            foreach ($tables as $table) {
                // Skip
                if(in_array($table,['employee_master_old','department_master','employee_master','emp_designation','shift_master','leave_master',' leave_type','leave_master','facility_master','holidays','attendance_policy','alog_summary','shift_master','attendance_log','emp_shiftlog','salary_structure','device_master','emp_docs','emp_nomination_detail','emp_facility','emp_education_detail','emp_docs_detail','canteen_appointment','canteen_trans','leave_master_old','account_setting'])){ continue; }

                $fields = $this->db->list_fields($table);
               
                if (in_array('emp_id', $fields)) {
                    $sql = "
                        UPDATE `$table` t
                        SET t.emp_id = 1 WHERE emp_id = 1051
                    ";
                    $this->db->query($sql);
                ...
                if (in_array('updated_by', $fields)) {
                    $sql = "
                        UPDATE `$table` t
                        SET t.updated_by = 1 WHERE updated_by = 1051
                    ";
                    $this->db->query($sql);
    ...
    /*** Migrate Employee_master_old_2 */

    public function migrateOldEmployeeFromV2(){
        try{
            $this->db->trans_begin();
            ...
            // $this->db->where('is_delete',0);
            $this->db->where('migrate_status',0);
            $result = $this->db->get('employee_master_old_2')->result();
            $i =0;$j=0;
            foreach($result as $row):
                $this->db->reset_query();
                $this->db->select("employee_master.*");
                $this->db->where('emp_code',$row->emp_code);
                $this->db->where('emp_name',$row->emp_name);
                $oldEmpData = $this->db->get('employee_master')->row();

                
                if(!empty($oldEmpData->id)){
                    $this->db->reset_query();
                    $this->db->where('id',$row->id);
                    $this->db->update('employee_master_old_2',['v2_emp_id' =>  $oldEmpData->id,'migrate_status'=>1]);
                    $i++;
                }
                $j++;
            endforeach;
    ...
    public function migrateOldEmpFromV2OnCode(){
        try{
            $this->db->trans_begin();
            ...
            foreach($result as $row):
                ...
                if(!empty($oldEmpData->id)){
                    $this->db->reset_query();
                    $this->db->where('id',$row->id);
                    $this->db->update('employee_master_old_2',['v2_emp_id' =>  $oldEmpData->id,'migrate_status'=>2]);
                    $i++;
                }else{
                    $this->db->reset_query();
                    $this->db->where('id',$row->id);
                    $this->db->update('employee_master_old_2',['migrate_status'=>3]);
                    $k++;
                }
                $j++;
            endforeach;
    ...
    public function migrateV2Password(){
        try{
            $this->db->trans_begin();

            $this->db->reset_query();
            $this->db->select("employee_master_v2.*");
            $this->db->where('emp_psc != 123456');
            $result = $this->db->get('employee_master_v2')->result();
            $i =0;$j=0;
            foreach($result as $row):
                $this->db->reset_query();
                $this->db->where('id',$row->id);
                $this->db->update('employee_master',['v2_emp_id' =>  $row->id,'emp_psc'=>$row->emp_psc,'emp_password'=>$row->emp_password]);
                $i++;
            endforeach;
            // exit;
            if($this->db->trans_status() !== FALSE):
