...
class MY_Controller extends CI_Controller{
	...
	public $deptCategory =  [0=>'',1=>"Dispatch",2=>"Engineering Design",3=>"Fettling",4=>"Heat Treatment",5=>"Melting",6=>"QC(Foundry Division)",7=>"Shelling",8=>"Stores",9=>"Wax Pattern",10=>"Account & Finance",11=>"Heat Treatment",12=>"Administration",13=>"Human Resource",14=>"Maintenance",15 =>"Procurement",16=>"QMS",17=>"Stores"];
	public $empRole = ["0"=>"","1"=>"Admin","2"=>"Production Manager","3"=>"Accountant","4"=>"Sales Manager","5"=>"Purchase Manager","6"=>"Employee","7"=>"HR"];
	...
	public $itemTypes = [1 => "Finish Goods", 2 => "Consumable", 3 => "Raw Material", 5 => "Machineries", 8 => "Services"/* , 4 => "Capital Goods", 6 => "Instruments", 7 => "Gauges", 9 => "Packing Material", 10 => "Scrap" */]; 
	...
	public function __construct(){
		...
		$this->load->model("RejectionTypeModel","rejectionType");
		$this->load->model("RejectionParameterModel","rejectionParameter");
		...
		$this->load->model('hr/AttendancePolicyModel', 'policy');
		$this->load->model('hr/HolidaysModel', 'holiday');
		...
		$this->load->model('hr/NoticeBoardModel','noticeBoard');
		$this->load->model('hr/IncrementPolicyModel','incrementPolicy');
		...
		$this->setSessionVariables(["masterModel","customField","customOption","dashboard","permission","terms","transport","hsnModel","itemCategory","item","department","designation","employeeCategory","shiftModel","employee","party","transMainModel","taxMaster","expenseMaster","salesEnquiry","salesOrder","purchaseOrder","store","storeLocation","gateInward","salesInvoice","purchaseInvoice","estimate","paymentVoucher","leads","salesQuotation","gstExpense","gstIncome","journalEntry","creditNote","debitNote","ebill","storeReport","salesReport","accountReport","gstReport","estimate","countries","packingList","commercialInvoice","customInvoice","instrument","empLoan","advanceSalary","leaveAuthorityModel","leave","leaveSetting","leaveApprove","requisition","issueRequisition","inspection","process","comment","sop","prcMaterialIssue","qcChallan","qcPRModel","qcIndent","qcPurchase","materialGrade","scrapGroup","purchaseIndent","rejectionReview","lineInspection","finalInspection","rejectionParameter","pos",'empRecruitment','biometric','attendance','manualAttendance','extraHours', 'policy', 'holiday','employeefacility','salaryStructure','skillMaster','noticeBoard','incrementPolicy','payroll',"deliveryChallan"]);
	...
	public function setSessionVariables($modelNames){
		...
		$this->data['start_year'] = $this->data['startYear'] = $this->startYear = date('Y',strtotime($this->dates[0]));
		$this->data['end_year'] = $this->data['endYear'] = $this->endYear = date('Y',strtotime($this->dates[1]));
		...
		$this->SEMI_FG_STORE = $this->session->userdata('SEMI_FG_STORE');
		...
		foreach($models as $modelName):
			...
			$this->{$modelName}->SEMI_FG_STORE = $this->SEMI_FG_STORE;
		endforeach;
		return true;
	}
