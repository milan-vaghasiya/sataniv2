...
class MasterModel extends CI_Model{
    ...
    public function pagingRows($data){
		...
		/********** Total Records without Filtering ***********/
		{
            ...
            if(!empty($data['cm_id'])):
    		    ...
    		    if(!empty($this->cm_id)){$this->db->where_in($data['tableName'].'.cm_id',[$this->cm_id,0]);}
            endif;
    ...
    public function row($data){
        ...
        if (isset($data['where'][$data['tableName'].'.'.'is_delete'])) { 
            $isDelete = $data['where'][$data['tableName'].'.'.'is_delete']; 
        } else { 
            $this->db->where($data['tableName'].'.is_delete',0);
        }
	...
	public function getCompanyInfo($cm_id=1){
		...
		$data['where']['company_info.id'] = ($cm_id!=0)?$cm_id:1;
    ...
    public function getGeneralSettings(){
        ...
        $queryData['where_in']['sub_menu_master.vou_name_short'] = ["'SOrd'","'POrd'","'GI'","'Sale'","'Purc'","'PckList'","'BCRct'","'Seq'","'Squot'","'GExp'","'GInc'","'Jrnl'","'C.N.'","'D.N.'","'COMINV'","'CUSTINV'"];
    ...
    */
	...
	/* Company List */
	public function getCompanyList($cm_id=""){
		return $this->db->where('is_delete',0)->get('company_info')->result();
	}

     /* Master Options */
	public function getMasterOptions(){
		return $this->db->where('id',1)->where('is_delete',0)->get('master_detail')->row();
	}	
