			...
			<div class="container">
				<div class="header-content">
					<div class="left-content">
						...
						<h5 class="title mb-0 text-nowrap"  id="desk_title">Inprogress PRC</h5>
					</div>
						...
						<!-- <div class="basic-dropdown">
							<div class="dropdown">
								...
								<div class="dropdown-menu" data-popper-placement="bottom-start" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(0px, 48px);">
									<a type="button" class="text-danger stageFilter  dropdown-item active" data-status="1"  data-postdata='{"status":"1","stage":"Pending prcs"}'>Pending prcs</a>
                                    <a type="button" class="text-danger stageFilter  dropdown-item" data-status="2"  data-postdata='{"status":"2","stage":"Completed prcs"}'>Completed prcs</a>
								</div>
							</div>
						</div>   -->
					</div>
        ...
        <div class="content-inner pt-0" >
			<div class="container">
                ...
                <div class="dz-tab style-4">
					<div class="list-grid">
						<ul id="prcContainer" class="dz-list message-list prcData accordion style-2"></ul>
					</div>
				</div> 
			</div>    
		</div>
...
$(document).ready(function(){
	...
	$(document).on('click','.stageFilter',function(){
		...
		postdata.page = 0;
		loadHtmlData({'fnget':'getPrcList','rescls':'prcData','postdata':postdata});
	});
	...
	$(scrollEle).scroll(function() {
		...
		if($(this).scrollTop() + $(this).innerHeight() >= ($(this)[0].scrollHeight - 10)) {
			if(ScrollDebounce){
				...
				loadHtmlData({'fnget':'getPrcList','rescls':'prcData','postdata':postdata,'scroll_type':1});
				setTimeout(function () { ScrollDebounce = true; }, 500);		
			}
		}
	});
	
	$(document).on("click", "ul.prcData li span.delete", function () {alert("delete");});
	$(document).on("click", "ul.prcData li span.flag", function () {alert("flag");});
	$(document).on("click", "ul.prcData li span.more", function () {alert("nothing");});

});

function prcResponse(data){
	if(data.status==1){	
