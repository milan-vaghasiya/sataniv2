        ...
        <div class="content-inner pt-0" >
			<div class="container">
                ...
                <div class="dz-tab style-4">
					<div class="list-grid">
						<ul id="leadContainer" class="dz-list message-list soData accordion style-2"></ul>
					</div>
				</div> 
			</div>    
		</div>
...
$(document).ready(function(){
	...
	$('.quicksearch').keyup(delay(function (e) {
			...
			loadHtmlData({'fnget':'getSalesOrderData','rescls':'soData','postdata':postdata});
	}));
	...
	$(scrollEle).scroll(function() {
		...
		if($(this).scrollTop() + $(this).innerHeight() >= ($(this)[0].scrollHeight - 10)) {
			if(ScrollDebounce){
				...
				loadHtmlData({'fnget':'getSalesOrderData','rescls':'soData','postdata':postdata,'scroll_type':1});
				setTimeout(function () { ScrollDebounce = true; }, 500);		
			}
		}
	});
	
	$(document).on("click", "ul.soData li span.delete", function () {alert("delete");});
	$(document).on("click", "ul.soData li span.flag", function () {alert("flag");});
	$(document).on("click", "ul.soData li span.more", function () {alert("nothing");});
